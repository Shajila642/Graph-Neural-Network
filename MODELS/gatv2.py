...
class GATv2Model(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, heads=1):
        ...
        self.conv2 = GATv2Conv(hidden_dim * heads, output_dim, heads=1)  # Final layer has 1 head
...
model = GATv2Model(input_dim=1, hidden_dim=64, output_dim=1, heads=4)
...
def train_gatv2(model, data, optimizer, criterion, epochs=50):
    model.train()
...
# Train the model
train_gatv2(model, graph_data_train, optimizer, criterion, epochs=50)

# Evaluation on the test set
def evaluate_gatv2(model, data):
    model.eval()
    with torch.no_grad():
        out = model(data)
        return out
...
y_pred = evaluate_gatv2(model, graph_data_test)
